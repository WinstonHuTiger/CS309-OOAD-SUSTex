...
diff_match_patch.prototype.diff_bisect_ = function (a, b, c) {
    ...
    for (var m = 0 != k % 2, p = 0, x = 0, w = 0, q = 0, t = 0; t < f && !((new Date).getTime() > c); t++) {
        for (var v = -t + p; v <= t - x; v += 2) {
            ...
            h[n] = r;
            if (r > d) x += 2; else if (y > e) p += 2; else if (m && (n = f + k - v, 0 <= n && n < g && -1 != l[n])) {
                var u = d - l[n];
                if (r >=
        ...
        for (v = -t + w; v <= t - q; v += 2) {
            ...
            l[n] = u;
            if (u > d) q += 2; else if (r > e) w += 2; else if (!m && (n = f + k - v, 0 <= n && n < g && -1 != h[n] && (r = h[n], y = f + r - n, u = d - u, r >= u))) return this.diff_bisectSplit_(a, b, r, y, c)
        }
    }
...
diff_match_patch.prototype.diff_halfMatch_ = function (a, b) {
    ...
    var f = this, g = c(d, e, Math.ceil(d.length / 4));
    d = c(d, e, Math.ceil(d.length / 2));
    if (g || d) g = d ? g ? g[4].length > d[4].length ? g : d : d : g; else return null;
...
diff_match_patch.prototype.match_bitap_ = function (a, b, c) {
    ...
    for (var k, m, p = b.length + a.length, x, w = 0; w < b.length; w++) {
        ...
        for (m[q + 1] = (1 << w) - 1; q >= k; q--) {
            ...
            if (m[q] & l && (t = d(w, q - 1), t <= g)) if (g = t, h = q - 1, h > c) k = Math.max(1, 2 * c - h); else break
        }
        if (d(w + 1, c) > g) break;
        x = m
    }
...
diff_match_patch.prototype.patch_make = function (a, b, c) {
    ...
    } else if (a && "object" == typeof a && "undefined" == typeof b && "undefined" == typeof c) b = a, d = this.diff_text1(b); else if ("string" == typeof a && b && "object" == typeof b && "undefined" == typeof c) d = a; else if ("string" == typeof a && "string" == typeof b && c && "object" == typeof c) d = a, b = c; else throw Error("Unknown call format to patch_make.");
...
diff_match_patch.prototype.patch_apply = function (a, b) {
    ...
    for (var d = 0, e = [], f = 0; f < a.length; f++) {
        var g = a[f].start2 + d, h = this.diff_text1(a[f].diffs), l = -1;
...
diff_match_patch.prototype.patch_addPadding = function (a) {
    ...
    d = a[0];
    var e = d.diffs;
    if (0 == e.length || e[0][0] != DIFF_EQUAL) e.unshift(new diff_match_patch.Diff(DIFF_EQUAL, c)), d.start1 -= b, d.start2 -= b, d.length1 += b, d.length2 += b; else if (b > e[0][1].length) {
...
diff_match_patch.prototype.patch_splitMax = function (a) {
    for (var b = this.Match_MaxBits, c = 0; c < a.length; c++) if (!(a[c].length1 <= b)) {
        ...
        for (var e = d.start1, f = d.start2, g = ""; 0 !== d.diffs.length;) {
            var h = new diff_match_patch.patch_obj, l = !0;
            h.start1 = e - g.length;
            h.start2 = f - g.length;
...
diff_match_patch.prototype.patch_fromText = function (a) {
    ...
    for (var c = 0, d = /^@@ -(\d+),?(\d*) \+(\d+),?(\d*) @@$/; c < a.length;) {
        ...
        for (c++; c < a.length;) {
            ...
            if ("-" == e) f.diffs.push(new diff_match_patch.Diff(DIFF_DELETE, g)); else if ("+" == e) f.diffs.push(new diff_match_patch.Diff(DIFF_INSERT, g)); else if (" " == e) f.diffs.push(new diff_match_patch.Diff(DIFF_EQUAL, g)); else if ("@" == e) break; else if ("" !== e) throw Error('Invalid patch mode "' + e + '" in: ' + g);
            c++
        }
    }
    return b
